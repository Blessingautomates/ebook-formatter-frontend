...
export const config = {
   ...
   * - Anything with a file extension: favicon.ico, robots.txt, and the static
   *   files under public/. A browser requests these anonymously, and bouncing
   *   them to /login would replace the asset with an HTML page.
   */
  matcher: [
    "/((?!api|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp|avif|ico|txt|xml|json|css|js|map|woff|woff2|ttf|otf)$).*)",
  ],
};
